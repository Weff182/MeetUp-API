...
describe('GET /api/users/auth', () => {
  beforeEach(async () => {
    myToken = await userServise.registartion(user.email, user.password);
    const decoded = jwt.verify(myToken, process.env.SECRET_KEY);
    id = decoded.id;
  });
  ...
  test('The function should return 200 status by valid myToken', async () => {
    const res = await request(app).get('/api/users/auth/').set('Authorization', `Bearer ${myToken}`);
    expect(res.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res.statusCode).toBe(200);
  });
  test('The function should return 200 status by valid myToken', async () => {
